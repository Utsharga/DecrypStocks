...
public class StockADT {
	...
	public StockADT(String filename, LocalDate targetDate) {
		...
		try (BufferedReader br = new BufferedReader(new FileReader("price-volume-data-for-all-us-stocks-etfs/Stocks/" + filename))) {
			String line = br.readLine(); // read over 1st line (header)
			System.out.println("Processing: " + this.name);
			// deals with empty files
			if (line == null) {
				this.growth = 0.0;
				this.price = 0.0;
				return;
